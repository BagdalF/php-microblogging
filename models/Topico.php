...
class Topico {
    public static function encontrarPostsPorTopico($idTopico) {
        ...
        $res->execute([':id_topico' => $idTopico]);
        return $res->fetchAll(PDO::FETCH_ASSOC);
    }

    public static function encontrarTopicos() {
        $banco = Banco::getConn();
        $res = $banco->query("SELECT * FROM topico");
        return $res->fetchAll(PDO::FETCH_ASSOC);
    }

    public static function encontrarTopicosDeUmUsuario($idUsuario) {
        $banco = Banco::getConn();
        $res = $banco->prepare("SELECT DISTINCT t.* FROM topico t
    ...
    public static function encontrarTopicoPorId($id_topico) {
        $banco = Banco::getConn();
        $res = $banco->prepare("SELECT * FROM topico WHERE id_topico = :id_topico LIMIT 1");
        $res->execute([':id_topico' => $id_topico]);
        return $res->fetch(PDO::FETCH_ASSOC);
    }

    public static function criarTopico($nome) {
        $banco = Banco::getConn();
        $res = $banco->prepare("INSERT INTO topico (nome) VALUES (:nome)");
        return $res->execute([':nome' => $nome]);
    }   

    public static function editarTopico($id_topico, $nome) {
        $banco = Banco::getConn();
        $res = $banco->prepare("UPDATE topico SET nome = :nome WHERE id_topico = :id_topico");
        return $res->execute([
            ':nome' => $nome,
            ':id_topico' => $id_topico
        ]);
    ...
    public static function apagarTopico($id_topico) {
        $banco = Banco::getConn();
        $res = $banco->prepare("DELETE FROM topico WHERE id_topico = :id_topico");
        return $res->execute([':id_topico' => $id_topico]);
    }
}
